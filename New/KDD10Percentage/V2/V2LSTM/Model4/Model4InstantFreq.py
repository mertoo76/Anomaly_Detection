...
#load x values
x_train=np.load("../../DataV2/Window100/instantFreq.npy")
x_test=np.load("../../DataV2/testData/instantFreq.npy")

#labels

tmp=pd.read_csv('../../../Data/kddcup.data_10_percent_corrected',',')
y_train = tmp.iloc[:, 41].values.copy()
...
y_test = tmp.iloc[:, 41].values.copy()
...
y_tmp=np.ones((x_train.shape[0],1))

for i in range(x_train.shape[0]):
    summ=sum(y_train[i*10:(i+1)*10])
    if summ != 10:
        y_tmp[i]=0
y_train=y_tmp.copy()

y_tmp=np.ones((x_test.shape[0],1))

for i in range(x_test.shape[0]):
    summ=sum(y_test[i*10:(i+1)*10])
    if summ != 10:
        y_tmp[i]=0
y_test=y_tmp.copy()
...
model = Sequential()
model.add(LSTM(64,batch_input_shape=(None,10,4),return_sequences=True))
